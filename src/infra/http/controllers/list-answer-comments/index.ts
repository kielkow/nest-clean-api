...
import {
  ListAnswerCommentsDTO,
  ListAnswerCommentsSchema,
} from '../../dtos/list-answer-comments.dto'
...
export class ListAnswerCommentsController {
  ...
  async handle(
    @Param('id') id: string,
    @Query(new ZodValidationPipe(ListAnswerCommentsSchema))
    params: ListAnswerCommentsDTO,
  ) {
    const perPage = 10
